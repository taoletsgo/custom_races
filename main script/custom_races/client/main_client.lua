    ...
    table.sort(sortedDrivers, function(a, b)
        if a.totalCheckpointsTouched == b.totalCheckpointsTouched then
			if a.hasnf ~= b.hasnf then
				...
				local aDist = #(GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(a.playerID))) - vector3(track.checkpoints[actualCheckPoint].x, track.checkpoints[actualCheckPoint].y, track.checkpoints[actualCheckPoint].z))
				local bDist = #(GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(b.playerID))) - vector3(track.checkpoints[actualCheckPoint].x, track.checkpoints[actualCheckPoint].y, track.checkpoints[actualCheckPoint].z))
				return aDist < bDist
			end
        else
	...
	Citizen.CreateThread(function()
		...
		for k, v in pairs(drivers) do
			...
			table.insert(racefrontpos, {
				...
				totaltimeNumber = v.hasnf and "NF" or v.totalRaceTime,
				bestLap = v.hasnf and "NF" or GetTimeAsString(v.bestLap)
			})
		end

		table.sort(racefrontpos, function(a, b)
			if a.totaltimeNumber == "NF" then
				return false
			elseif b.totaltimeNumber == "NF" then
				return true
			else
				return a.totaltimeNumber < b.totaltimeNumber
			end
		end)

		for i = 1, #racefrontpos do
			racefrontpos[i].position = i
		end
